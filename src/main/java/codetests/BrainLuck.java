...
public class BrainLuck {
    ...
    public String process(String input) {
        int[] band = new int[2000];
        Arrays.stream(band).forEach(entry -> entry = (short) 0);
        int[] inputChars = input.chars().toArray();
        int dataPointer = 0, inputPointer = 0;
        String result = "";
        char[] code = this.code.toCharArray();
        for (int instrPointer = 0; instrPointer < this.code.toCharArray().length; instrPointer++) {
            switch (code[instrPointer]) {
                case '>':
                    dataPointer++;
                    break;
                case '<':
                ...
                case '+':
                    ...
                    break;
                case '-':
                    if (band[dataPointer] > 0) {
                        band[dataPointer]--;
                    } else {
                        band[dataPointer] = 255;
                    }
                    break;
                case '.':
                    result+= (char) band[dataPointer];
                    break;
                case ',':
                    band[dataPointer] = inputChars[inputPointer];
                    inputPointer++;
                    break;
                case '[':
                    if (band[dataPointer] == 0) {
                        int openingBraces = 1;
                        int i = 0;
                        while (i != openingBraces) {
                            while (code[instrPointer] != ']') {
                                instrPointer++;
                                if (code[instrPointer] == '[') openingBraces++;
                            }
                            instrPointer++;
                            i++;
                        }
                ...
                case ']':
                    if (band[dataPointer] != 0) {
                        int closingBraces = 1;
                        int j = 0;
                        while (j != closingBraces) {
                            while (code[instrPointer] != '[') {
                                instrPointer--;
                                if (code[instrPointer] == ']') closingBraces++;
                            }
                            instrPointer--;
                            j++;
                        }
